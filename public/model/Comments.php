...
class Comments{
    ...
    $query = "SELECT * FROM comments WHERE disk_id=".(string)$id." ORDER BY id DESC";
    $db = new Database();
    $arr = $db->getAll($query);
    return $arr;
}
...
public static function getCommentsCountByDiskID($id) {
    $query = "SELECT count(id) as 'count' FROM comments WHERE disk_id=".(string)$id; $db = new Database();
    $c = $db->getOne($query);
    return $c;
    }
